    ...
    _browserPopulate: function (initialAssetsQueryObj, relationsQueryObj, cb) {
        ...
        async.eachLimit(initialAssets, 1, function (initialAsset, cb) {
            ...
            that.eachAssetPreOrderAsync(initialAsset, relationsQueryObj, function preOrderCb(asset, incomingRelation, cb) {
                ...
                } else if (asset.type === 'RequireJsConfig') {
                    if (incomingRelation.isRequireVar) {
                        if (context.require) {
                        ...
                        if (context.require) {
                            new vm.Script('require.config(' + asset.parseTree.print_to_string() + ');', asset.urlOrDescription).runInContext(context);
                        } else {
                ...
                if (!asset.isPopulated) {
                    asset.outgoingRelations.forEach(function (outgoingRelation) {
                        if (!outgoingRelation.assetGraph) {
                            if (!outgoingRelation.to && (outgoingRelation.type === 'JavaScriptAmdDefine' || outgoingRelation.type === 'JavaScriptAmdRequire') && outgoingRelation.hrefType === 'moduleName') {
                                outgoingRelation.to = {
                                    url: context.require.toUrl(outgoingRelation.href + '.js')
                                };
                            }
