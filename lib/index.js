                ...
                if (outgoingRelation.hrefType === 'moduleName') {
                    ...
                    for (var i = 0 ; i < outgoingRelation.contexts.length ; i += 1) {
                        if (outgoingRelation.contexts[i] === context) {
                            hasThisContext = true;
                        } else if (outgoingRelation.contexts[i].vmContext.require && outgoingRelation.contexts[i].vmContext.require.toUrl(moduleName) !== url) {
                            throw new Error('The module name ' + moduleName + ' resolves to different urls in different contexts');
                        }
                    }
                }
            } else {
