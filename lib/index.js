...
function hey(moduleName, context, outgoingRelation) {
    var url = context.vmContext.require.toUrl(moduleName);
    if (!outgoingRelation.contexts) {
        outgoingRelation.contexts = [];
    } else if (outgoingRelation.contexts.some(function (otherContext) {
        return otherContext.vmContext.require.toUrl(moduleName) !== url;
    })) {
        throw new Error('vqwevqwevqwe');
    };
    if (outgoingRelation.contexts.indexOf(context) === -1) {
        outgoingRelation.contexts.push(context);
    }
    console.log("contexts after", outgoingRelation.contexts.length);
    return url;
}
...
console.log("cloneAndExpand", context.id)
            context = context.cloneAndExpand(asset);
console.log("into", context.id)
        }
        asset.outgoingRelations.forEach(function (outgoingRelation) {
            if (!outgoingRelation.assetGraph) { // Won't work when there're multiple initial assets
                if (!outgoingRelation.to && (outgoingRelation.type === 'JavaScriptAmdDefine' || outgoingRelation.type === 'JavaScriptAmdRequire' || outgoingRelation.type === 'JavaScriptShimRequire')) {
                    ...
                    outgoingRelation.to = {
                        url: hey(href, context, outgoingRelation)
                    };
                }
                ...
                if (outgoingRelation.to.url || typeof outgoingRelation.to === 'string') {
                    ...
                    var relativeUrl;
                    if (outgoingRelation.hrefType === 'moduleName') {
                        relativeUrl = hey(outgoingRelation.href, context, outgoingRelation);
                    } else {
