...
//        asset.populate();
    ...
    _traverseAsync: function (startAssetOrRelation, relationQueryObj, preOrderLambda, postOrderLambda, cb) {
        ...
        (function traverse(asset, incomingRelation, cb) {
            ...
            preOrderLambda(asset, incomingRelation, function () {
                seenAssets[asset.id] = true;
                assetStack.push(asset);
                var outgoingRelations = that.findRelations({from: asset});
                async.eachLimit(outgoingRelations, 1, function (relation, cb) {
                    if (!relationQueryMatcher || relationQueryMatcher(relation)) {
                        traverse(relation.to, relation, cb);
                    } else {
                        setImmediate(cb);
                    }
                }, function () {
        ...
        async.eachLimit(initialAssets, 1, function (initialAsset, cb) {
            ...
            that.eachAssetPreOrderAsync(initialAsset, relationsQueryObj, function preOrderCb(asset, incomingRelation, cb) {
                if (!asset.isLoaded) {
                    return asset.load(function (err) {
                        if (err) {
                            ...
                            postOrderCb(asset, incomingRelation, cb);
                        }
                    });
                }
                ...
                if (!asset.isPopulated) {
                    if (asset.type === 'JavaScript' && asset.fileName === 'require.js') {
                        seenRequireJs = true;
                        new vm.Script(asset.text, asset.urlOrDescription).runInContext(context);
                    } else if (asset.type === 'JavaScriptRequireJsConfig') {
                        if (incomingRelation.isVarRequire) {
                            if (seenRequireJs) {
                                that.emit('warn', new Error('Saw var require={...} config object after require.js'));
                            } else {
                                new vm.Script('var require = ' + asset.text + ';', asset.urlOrDescription).runInContext(context);
                            }
                        } else {
                            if (seenRequireJs) {
                                new vm.Script('require.config(' + asset.text + ');', asset.urlOrDescription).runInContext(context);
                            } else {
                                that.emit('warn', new Error('Saw require.config({...}) or requirejs.config({...}) without require.js being detected'));
                            }
                        }
                        new vm.Script(asset.text, asset.urlOrDescription).runInContext(context);
                    }
                    asset.outgoingRelations.forEach(function (outgoingRelation) {
                        if (outgoingRelation.type === 'JavaScriptRequireJsConfig') {
                            if (outgoingRelation.isRequireVar) {
                                new vm.Script(asset.text, asset.urlOrDescription).runInContext(context);
                            } else {
                                new vm.Script(asset.text, asset.urlOrDescription).runInContext(context);
                            }
                            context.requirejs.config(outgoingRelation.to.parseTree);
                        } else if (!outgoingRelation.assetGraph) {
                            if (outgoingRelation.to.url || typeof outgoingRelation.to === 'string') {
                                ...
                                if (outgoingRelation.hrefType === 'moduleName') {
                                    relativeUrl = context.require.toUrl(outgoingRelation.href);
                                } else {
